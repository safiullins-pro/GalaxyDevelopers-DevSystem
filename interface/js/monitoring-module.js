...
class GalaxyMonitoringModule {
            ...
            <div class="monitoring-content" id="monitoring-content">
                ...
                <div class="mon-section">
                    ...
                    <div class="status-grid">
                        ...
                        <div class="status-item">
                            <span class="status-indicator" id="ai-protection-status"></span>
                ...
                <div class="mon-section">
                    ...
                    <div class="action-buttons">
                        ...
                        <button class="mon-btn" onclick="galaxyMonitoring.scanThreats()">
                            🛡️ Сканирование угроз
                        </button>
                        <button class="mon-btn" onclick="galaxyMonitoring.checkFileProtection()">
                            📋 Проверка защиты
                        </button>
    ...
    updateStatusIndicators() {
        ...
        const aiProtectionStatus = document.getElementById('ai-protection-status');
        ...
        if (aiProtectionStatus) {
            aiProtectionStatus.className = `status-indicator ${this.componentStatus.aiProtection || 'offline'}`;
        }
    }
    ...
    updateSystemStatus(data) {
        ...
        // Обновляем статус AI Protection
        if (data.ai_protection) {
            this.componentStatus.aiProtection = data.ai_protection.enabled ? 'online' : 'offline';
        }
    ...
    // ========== AI PROTECTION ==========
    async scanThreats() {
        this.showNotification('Запуск сканирования угроз...', 'info');
        
        try {
            const response = await this.fetchAPI('/api/protection/scan-threats', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    directory: '/Volumes/Z7S/development/GalaxyDevelopers/DevSystem'
                })
            });
            
            const threats = response.threats || [];
            if (threats.length > 0) {
                this.showNotification(`⚠️ Найдено ${threats.length} угроз!`, 'warning');
                console.log('Найденные угрозы:', threats);
            } else {
                this.showNotification('✅ Угрозы не найдены', 'success');
            }
            
            return response;
        } catch (error) {
            this.showNotification('Ошибка сканирования угроз', 'error');
    ...
    async checkFileProtection() {
        try {
            const response = await this.fetchAPI('/api/protection/status');
            
            const enabled = response.protection_enabled;
            const status = response.ai_auditor_status;
            
            if (enabled) {
                this.showNotification(`✅ AI Protection активен (${status})`, 'success');
            } else {
                this.showNotification('⚠️ AI Protection недоступен', 'warning');
            }
            
            return response;
        } catch (error) {
            this.showNotification('Ошибка проверки защиты', 'error');
    ...
    async auditCode(code, filePath = 'unknown') {
        try {
            const response = await this.fetchAPI('/api/protection/audit-code', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    code: code,
            ...
            const riskLevel = response.overall_risk;
            const issues = response.issues_found || 0;
            
            if (riskLevel === 'critical' || riskLevel === 'high') {
                this.showNotification(`🚨 Высокий риск! Найдено ${issues} проблем`, 'error');
            } else if (issues > 0) {
                this.showNotification(`⚠️ Найдено ${issues} проблем (${riskLevel})`, 'warning');
            } else {
                this.showNotification('✅ Код безопасен', 'success');
            }
            
            return response;
        } catch (error) {
            this.showNotification('Ошибка аудита кода', 'error');
